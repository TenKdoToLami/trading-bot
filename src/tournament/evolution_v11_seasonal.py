...
import os
import numpy as np
from strategies.genome_v11_seasonal import GenomeV11Seasonal
...
def _init_worker(cache_file):
    ...
    # Include credit_spread in the records
    _worker_price_data = df[['open', 'high', 'low', 'close', 'volume', 'vix', 'yield_curve', 'credit_spread']].to_dict('records')

def _evaluate_v11_seasonal_worker(genome):
    from contextlib import redirect_stdout
    import os
    with open(os.devnull, 'w') as fnull:
        with redirect_stdout(fnull):
            res = _execute_simulation(
                strategy_type=GenomeV11Seasonal,
...
@register_evolution("v11_seasonal")
class EvolutionEngineV11Seasonal(BaseEvolutionEngine):
    def __init__(self, **kwargs):
        ...
        super().__init__(version_id="v11_seasonal", **kwargs)

    def _random_genome(self):
        layers = []
        # 18 Inputs -> 32 Hidden -> 4 Outputs
        layers.append({'w': (np.random.randn(18, 32) * 0.5).tolist(), 'b': (np.random.randn(32) * 0.1).tolist()})
        layers.append({'w': np.random.uniform(-1, 1, (32, 4)).tolist(), 'b': np.random.uniform(-0.1, 0.1, 4).tolist()})
        return {
            'version': 'v11_seasonal', 'layers': layers,
            'lookbacks': {k: random.randint(mn, mx) for k, (mn, mx) in self.lb_bounds.items()},
            'hysteresis': random.uniform(0.1, 0.4), 'smoothing': random.uniform(0.2, 0.7)
        }

    def _mutate(self, genome):
        mutated = json.loads(json.dumps(genome))
        mutated['version'] = 'v11_seasonal'
        ...
        if random.random() < self.mut_rate: 
            mutated['hysteresis'] = max(0.01, min(0.8, mutated['hysteresis'] + random.gauss(0, 0.05 * self.mut_strength)))
        if random.random() < self.mut_rate: 
            mutated['smoothing'] = max(0.01, min(0.99, mutated['smoothing'] + random.gauss(0, 0.1 * self.mut_strength)))
        return mutated

    def _get_worker_config(self):
        return _evaluate_v11_seasonal_worker, (_init_worker, (CACHE_FILE,))
