        ...
        for g in ['panic', 'base']:
            for k in self.indicators:
                if random.random() < self.mut_rate: 
                    mut[f'{g}_weights'][k] += random.gauss(0, 0.5 * self.mut_strength)
                if self.use_ablation and random.random() < 0.05: 
                    mut[f'{g}_active'][k] = not mut[f'{g}_active'][k]
                    
        if random.random() < self.mut_rate: 
            mut['panic_threshold'] += random.gauss(0, 0.5 * self.mut_strength)
        if random.random() < self.mut_rate: 
            mut['lock_days'] = max(0, min(20, mut['lock_days'] + random.gauss(0, 2 * self.mut_strength)))
        return mut
