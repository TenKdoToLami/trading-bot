...
class EvolutionEngineV1Manual(BaseEvolutionEngine):
    ...
    def _mutate(self, genome):
        ...
        if random.random() < self.mut_rate: 
            mut['sma'] = max(50, min(500, mut['sma'] + int(random.gauss(0, 20 * self.mut_strength))))
        if random.random() < self.mut_rate: 
            mut['min_b_days'] = max(1, min(60, mut['min_b_days'] + int(random.gauss(0, 2 * self.mut_strength))))
        if random.random() < self.mut_rate: 
            mut['bounds_p'] = sorted([max(5, min(100, b + random.gauss(0, 5 * self.mut_strength))) for b in mut['bounds_p']])
        for i in range(len(mut['weights_p'])):
            for j in range(len(mut['weights_p'][i])):
                if random.random() < self.mut_rate: 
                    mut['weights_p'][i][j] = max(0, min(1.0, mut['weights_p'][i][j] + random.gauss(0, 0.1 * self.mut_strength)))
        return mut
