...
def _evaluate_v13_moe_full_worker(genome):
...
@register_evolution("v13_moe_full")
class EvolutionV13MOEFull(BaseEvolutionEngine):
    def __init__(self, **kwargs):
        ...
        super().__init__(version_id="v13_moe_full", **kwargs)

    def _get_worker_config(self) -> Tuple[Callable, Tuple]:
        return _evaluate_v13_moe_full_worker, (_init_worker_lazy, ())
    ...
    def _mutate(self, genome: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if random.random() < rate:
            g['smoothing'] = np.clip(g['smoothing'] * random.uniform(0.9, 1.1), 0.05, 0.8)
        if random.random() < rate:
            g['hysteresis'] = np.clip(g['hysteresis'] * random.uniform(0.9, 1.1), 0.01, 0.5)
    ...
    def _crossover(self, g1: Dict[str, Any], g2: Dict[str, Any]) -> Dict[str, Any]:
        ...
        child['hysteresis'] = (g1['hysteresis'] + g2['hysteresis']) / 2
        child['temp'] = (g1.get('temp', 1.0) + g2.get('temp', 1.0)) / 2
        return child
