    ...
    def _mutate(self, genome):
        ...
        for b in self.brains:
            for k, v in mut['brains'][b]['w'].items():
                if random.random() < self.mut_rate: 
                    mut['brains'][b]['w'][k] += random.gauss(0, 0.8 * self.mut_strength)
                if self.use_ablation and random.random() < 0.05: 
                    mut['brains'][b]['a'][k] = not mut['brains'][b]['a'][k]
            if random.random() < self.mut_rate:
                t = mut['brains'][b].get('t', 0.0)
                mut['brains'][b]['t'] = t + random.gauss(0, 0.5 * self.mut_strength)
        if random.random() < self.mut_rate: 
            mut['temp'] = max(0.1, min(3.0, mut['temp'] + random.gauss(0, 0.1 * self.mut_strength)))
        if random.random() < self.mut_rate: 
