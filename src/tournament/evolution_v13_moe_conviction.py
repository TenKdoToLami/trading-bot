...
def _evaluate_v13_moe_conviction_worker(genome):
    """Universal worker for parallel evaluation."""
    _init_worker_lazy()
...
@register_evolution("v13_moe_conviction")
class EvolutionV13MOEConviction(BaseEvolutionEngine):
    def __init__(self, **kwargs):
        ...
        super().__init__(version_id="v13_moe_conviction", **kwargs)

    def _get_worker_config(self) -> Tuple[Callable, Tuple]:
        return _evaluate_v13_moe_conviction_worker, (_init_worker_lazy, ())
    ...
    def _random_genome(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'temp': 1.0,
            'skew_power': random.uniform(1.0, 4.0),
            'conviction_cap': random.uniform(0.75, 0.98),
    ...
    def _mutate(self, genome: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if random.random() < rate:
            g['smoothing'] = np.clip(g.get('smoothing', 0.3) * random.uniform(0.9, 1.1), 0.05, 0.8)
        if random.random() < rate:
            g['skew_power'] = np.clip(g.get('skew_power', 2.0) * random.uniform(0.9, 1.1), 1.0, 5.0)
        if random.random() < rate:
            g['conviction_cap'] = np.clip(g.get('conviction_cap', 0.9) * random.uniform(0.95, 1.05), 0.7, 0.99)
    ...
    def _crossover(self, g1: Dict[str, Any], g2: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Safe access with defaults for cross-version seeding
        child['smoothing'] = (g1.get('smoothing', 0.3) + g2.get('smoothing', 0.3)) / 2
        child['skew_power'] = (g1.get('skew_power', 2.0) + g2.get('skew_power', 2.0)) / 2
        child['conviction_cap'] = (g1.get('conviction_cap', 0.9) + g2.get('conviction_cap', 0.9)) / 2
        return child
