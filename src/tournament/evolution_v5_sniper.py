...
class EvolutionEngineV5Sniper:
    ...
    def _random_genome(self):
        ...
        return {'sniper': {'w': {k: random.uniform(-4, 4) for k in self.indicators}, 'a': {k: (random.random() > 0.4 if self.use_ablation else True) for k in self.indicators}, 't_low': t1, 't_high': t2, 'lookbacks': {k: random.randint(mn, mx) for k, (mn, mx) in self.lb_bounds.items()}}, 'lock_days': random.uniform(1, 10), 'version': 5.0}
    ...
    def run(self):
        ...
        print(f"Starting V5S Evolution: {self.generations} gens, pop {self.pop_size}, mut {self.mut_rate:.2f}")
        ...
        with concurrent.futures.ProcessPoolExecutor(initializer=_init_worker, initargs=(CACHE_FILE,)) as executor:
            for gen in range(self.generations):
                ...
                futures = [executor.submit(_evaluate_v5s_worker, g) for g in self.population]
                scored = []
                ...
                if cagr > (self._best_seen["cagr"] + 0.1) or dd < (self._best_seen["dd"] - 0.5):
                    ...
                    v_path = os.path.join(vault_dir, f"v5s_cagr_{cagr:.1f}_dd_{dd:.1f}.json")
