...
def _init_worker(cache_file, min_cagr):
    ...
    print(f"  [Worker {os.getpid()}] V7 Binary Ready. Min CAGR: {_worker_min_cagr*100:.1f}%")

def _evaluate_genome_worker(genome):
    res = _execute_simulation(
        strategy_type=GenomeV7DeepBinary,
...
class EvolutionEngineV7DeepBinary:
    def __init__(self, population_size=50, generations=20, mutation_rate=0.2, seed_vault=None, min_cagr=0.0):
        ...
        print("Loading master data for Evolution V7 Binary...")
        ...
        return {
            ...
            'layers': [
                ...
                },
                {
                    ...
                    'b': np.random.uniform(-0.1, 0.1, 2).tolist()
                }
            ],
    ...
    def _crossover(self, p1, p2):
        ...
        for i in range(len(p1['layers'])):
            if random.random() > 0.5:
                ...
                child['layers'].append(p2['layers'][i])
        return child
    ...
    def run(self):
        ...
        print(f"Starting Evolution V7 Binary: {self.generations} generations, pop {self.population_size}")
            ...
            for gen in range(self.generations):
                ...
                if best_fit > best_overall_fitness:
                    ...
                    if best_metrics['cagr'] >= self.min_cagr:
                        ...
                        with open(f"{vault_dir}/v7db_cagr_{c:.2f}_dd_{d:.2f}.json", "w") as f:
                            json.dump(best_genome, f, indent=2)
                        save_path = "champions/v7_deep_binary/genome.json"
                        os.makedirs(os.path.dirname(save_path), exist_ok=True)
                        with open(save_path, "w") as f: json.dump(best_genome, f, indent=2)
                
                elapsed = time.time() - start_time
                print(f"V7-Bin  Gen {gen+1:02d} | Fit: {best_fit:6.2f} | CAGR: {best_metrics['cagr']*100:6.2f}% | MaxDD: {best_metrics['max_dd']*100:6.2f}% | Time: {elapsed:.1f}s")
