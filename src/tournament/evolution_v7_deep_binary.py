...
def _init_worker(cache_file, min_cagr):
    ...
    print(f"  [Worker {os.getpid()}] V7-Binary Ready. Min CAGR: {min_cagr:.1f}%")
...
class EvolutionEngineV7DeepBinary:
    def __init__(self, population_size=50, generations=20, mutation_rate=0.2, seed_vault=None, min_cagr=0.0):
        ...
        if seed_vault and os.path.exists(seed_vault):
            ...
            print(f"  SUCCESS: Injected {num_seeds} V7 Binary seeds from vault.")
    ...
    def run(self):
        ...
        print(f"Starting Evolution V7 Binary: {self.generations} generations, pop {self.population_size}, mut {self.mutation_rate:.2f}, MinCAGR: {self.min_cagr:.1f}%")
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, initializer=_init_worker, initargs=(self.cache_file, self.min_cagr)) as executor:
            for gen in range(self.generations):
                ...
                if (best_metrics['cagr'] * 100) >= self.min_cagr:
                    v_path = os.path.join(vault_dir, f"v7db_cagr_{best_metrics['cagr']*100:.2f}_dd_{best_metrics['max_dd']*100:.2f}.json")
