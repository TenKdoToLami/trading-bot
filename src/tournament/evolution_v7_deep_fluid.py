...
def _evaluate_v7df_worker(genome):
...
@register_evolution("v7_deep_fluid")
class EvolutionEngineV7DeepFluid(BaseEvolutionEngine):
    def __init__(self, **kwargs):
        ...
        super().__init__(version_id="v7_deep_fluid", **kwargs)

    def _random_genome(self):
        return {'version': 'v7_deep_fluid', 'layers': [{'w': np.random.uniform(-1, 1, (13, 24)).tolist(), 'b': np.random.uniform(-0.1, 0.1, 24).tolist()}, {'w': np.random.uniform(-1, 1, (24, 4)).tolist(), 'b': np.random.uniform(-0.1, 0.1, 4).tolist()}], 'lookbacks': {k: random.randint(mn, mx) for k, (mn, mx) in self.lb_bounds.items()}, 'lock_days': random.uniform(1, 10), 'rebalance_threshold': random.uniform(0.01, 0.10)}
    ...
    def _mutate(self, genome):
        ...
        if random.random() < self.mut_rate:
            mut['rebalance_threshold'] = max(0.01, min(0.25, mut['rebalance_threshold'] + random.gauss(0, 0.02 * self.mut_strength)))
        return mut

    def _get_worker_config(self):
        return _evaluate_v7df_worker, (_init_worker, (CACHE_FILE,))
