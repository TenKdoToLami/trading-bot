"""
Genome V13.11 MOE "Adaptive VIX"
Version 13.11 - Volatility-Aware Conviction
1. Sentinel (The Decider): Outputs Bull/Bear probabilities.
2. Adaptive Kill Switch: Threshold scales with VIX. 
   - Low VIX = Aggressive 100/0 switch.
   - High VIX = Safe Asset Splits (Hedging).
"""
...
@register_strategy(["v13_moe_vix", 13.11])
class GenomeV13MOEVIX(BaseStrategy):
    NAME = "Genome V13.11 (MOE - Adaptive VIX)"
    version = 13.11
    ...
    def _default_genome(self):
        ...
        return {
            ...
            'temp': 1.0,
            'skew_power': 2.0,
            'vix_base_cap': 0.85,    # Base threshold for 100% switch
            'vix_sensitivity': 0.01  # How much the threshold rises per point of VIX
        }
    ...
    def on_data(self, date, price_data, prev_data):
        ...
        # --- ADAPTIVE VIX KILL SWITCH ---
        # Threshold rises as VIX rises (e.g. VIX 15 -> 0.85, VIX 30 -> 1.0)
        vix_base = self.genome.get('vix_base_cap', 0.85)
        vix_sens = self.genome.get('vix_sensitivity', 0.01)
        dynamic_cap = vix_base + (max(macro_vix, 15.0) - 15.0) * vix_sens
        
        final_probs = self.smoothed_regime.copy()
        if np.max(final_probs) > dynamic_cap:
