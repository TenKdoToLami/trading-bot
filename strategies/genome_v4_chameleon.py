...
class ChameleonV4(BaseStrategy):
    ...
    def on_data(self, date: str, price_data: dict, prev_data=None) -> dict:
        ...
        # Get activation mask (default all ON)
        ab = self.genome.get('a', {'vix': True, 'mom': True, 'rsi': True})

        # 1. Volatility Regime (Relative Stretch)
        vix_baseline = sum(self.vix[-self.genome["vix_ema"]:]) / self.genome["vix_ema"]
        # If vix ablation is OFF, vol_panic is never triggered
        vol_panic = (price_data['vix'] > (vix_baseline * self.genome["vol_stretch"])) if ab.get('vix', True) else False
        
        # 2. Momentum Filter
        mom_sma = sum(self.closes[-self.genome["mom_period"]:]) / self.genome["mom_period"]
        # If mom ablation is OFF, trend is always considered UP
        trend_up = (price_data['close'] > mom_sma) if ab.get('mom', True) else True
        
        # 3. Mean Reversion (RSI)
        r_val = rsi(self.closes, self.genome["rsi_period"], state=self.rsi_state)
        # If rsi ablation is OFF, dips are never detected
        dip_detected = ((r_val or 50) < self.genome["rsi_entry"]) if ab.get('rsi', True) else False
