"""
Genome V7 Strategy — Deep Brain (Neuroevolution).
Uses a Multi-Layer Perceptron (MLP) to process indicators.
Capable of learning non-linear relationships between market signals.
"""
...
class GenomeV7Deep(BaseStrategy):
    ...
    def on_data(self, date, price_data, prev_data):
        ...
        # 1. Calculate Indicators (using genome-specific lookbacks)
        ...
        # 3. Deep Brain Inference (MLP Forward Pass)
        # Layer 1: Input -> Hidden (ReLU)
        h1 = self._relu(np.dot(inputs, self.w1) + self.b1)
        
        # Layer 2: Hidden -> Output (Raw Scores)
        scores = np.dot(h1, self.w2) + self.b2
        
        # 4. Decision (Highest Score Wins)
        # Order: 0=CASH, 1=1x, 2=2x, 3=3x
        winner_idx = np.argmax(scores)
        
        if winner_idx == 0: new_holdings = {"CASH": 1.0}
        elif winner_idx == 1: new_holdings = {"SPY": 1.0}
        elif winner_idx == 2: new_holdings = {"2xSPY": 1.0}
